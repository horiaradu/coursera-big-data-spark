...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = columnNames match {
    case head :: tail => StructType(
      StructField(head, StringType, nullable = false) ::
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val workPrefixes = List("t05", "t1805", "t11", "t1801", "t1803")
    ...
    columnNames.foldLeft((List.empty[Column], List.empty[Column], List.empty[Column])) {
      case ((primary, work, other), column) =>
        if (primaryNeedsPrefixes.exists(column.startsWith)) (col(column) :: primary, work, other)
        else if (workPrefixes.exists(column.startsWith)) (primary, col(column) :: work, other)
        else if (otherPrefixes.exists(column.startsWith)) (primary, work, col(column) :: other)
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    def round(value: Double) = (value * 10).round / 10d
